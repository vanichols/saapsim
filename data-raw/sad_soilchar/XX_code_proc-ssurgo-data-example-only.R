...
#                4/30/2020 (want to add SOC from SSURGO to see why IACSR isn't related in FACTS data)
#
# purpose: use feddata package to get ssurgo data?
# notes: something is wrong with the OM weighted value....


rm(list = ls())
library(saapsim) #--for sad_siteinfo
library(tidyverse)
library(janitor)
library(lubridate)
library(sf)
...
fun_summarize_ssurgo <- function(f_mychoice = "ames") {
  ...
  om_dat <-
    ...
    select(cokey,
           ...
           hzname,
           desgnmaster,
           contains(".r") & contains("om")) %>%
    filter(desgnmaster == "A") %>%
    #--average by cokey (?)
    group_by(cokey) %>%
    summarise(om = mean(om.r, na.rm = T))
...
dat %>%
  ...
  write_csv("td_ssurgo-vals.csv")

dat %>%
  select(site, everything()) %>%
  write_csv("../../../Documents/_git_repos/ghproj_ccgap/01_create-features/raw_ssurgo-vals.csv")
